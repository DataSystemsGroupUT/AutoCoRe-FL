...
def main():
    base_config_path = "/gpfs/helios/home/soliman/logic_explained_networks/experiments/AutoCore_FL/configs/config_lrxfl_fl_autocore_ade20k.yaml"
    ...
    # Also seed numpy and random for other parts if needed (AutoCoRe part already does this)
    np.random.seed(SEED); random.seed(SEED); torch.manual_seed(SEED)
    
    torch_device_main = torch.device(config['device'])
    ...
    # global_model_parameters_list = [] (though not used for decisions in snippet)
    ...
    main_logger.info(f"======== FINAL Global LR-XFL Model Accuracy using AutoCore Concepts for Run ID: {run_id} ========")
    ...
    with open(output_log_filename, 'a') as flog_final_summary: 
        ...
        flog_final_summary.write(f'Global LR-XFL Model Accuracy using AutoCore Concepts: {final_lrxfl_model_test_acc:.4f}\n')
