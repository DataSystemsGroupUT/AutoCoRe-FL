...
try:
    ...
    from AutoCore_FL.federated.utils import setup_logging,  save_config, load_config, SAM2Filter, evaluate_global_AutoCore_model, load_labels_from_manifest, generate_run_id
    from AutoCore_FL.concepts.vectorizer import build_image_concept_vectors

except ImportError as e:
    logging.basicConfig(level=logging.ERROR)
    logging.error(f"Critical Import Error in run_autocore_fl_from_cached_data.py: {e}. Execution cannot continue.")
    sys.exit(1)
...
def main_autocore_fl():
    config_path = "/gpfs/helios/home/soliman/logic_explained_networks/experiments/AutoCore_FL/configs/config_autocore_fl_auto_ade20k.yaml" # Create this YAML config file
    if not os.path.exists(config_path):
        print(f"ERROR: Config file {config_path} not found. Please create it.")
       
        return
    config = load_config(config_path)
    run_id = generate_run_id(config.get('method_name', 'AutoCoReFL'))
    ...
    main_logger.info(f"======== Starting AutoCoRe-FL: {run_id} ========")
    ...
    for i in range(num_configured_clients):
        ...
        # data_partition argument is now not directly used by client's __init__ for loading
        # Instead, client will load its own data using client_id and paths from client_config_copy
    ...
    # Filter initial concepts based on support from the LAST successful K-Means round
    ...
    # --- Server Holdout Data Vectorization ---
    ...
    if server_X_val_concepts is not None and server_y_val_labels is not None:
        server.server_X_test_concepts_for_validation = server_X_val_concepts
    ...
    # --- PHASE 3: Federated Rule Learning (GBM-Style) ---
    main_logger.info(f"--- Starting Phase 3: Federated Rule Learning (GBM-Style) ---")
    main_logger.info(f"GBM Learning Rate: {config.get('learning_rate_gbm', 0.1)}") # Corrected key
    ...
    for r_fl in range(config["phase2_rounds"]): # Phase2_rounds is for FL rule learning
        ...
        main_logger.info(f"--- FIGS FL Round {r_fl + 1}/{config['phase2_rounds']} ---")
    ...
    main_logger.info(f"  Num Clients: {num_configured_clients}, Num AutoCoRe-FL Features: {num_figs_features}")
    main_logger.info(f"  Global AutoCoRe-FL Model Accuracy: {model_acc:.4f}")
    main_logger.info(f"  Global AutoCoRe-FL Rule Metrics: P={rule_p:.3f}, C={rule_c:.3f}, L={rule_l:.2f}, F={rule_f:.3f}")
