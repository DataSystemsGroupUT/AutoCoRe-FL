...
# --- Configuration ---
# Paths to the pre-computed ResNet18 concept predictions
# These should point to the *output* of 2_generate_predicted_concept_features.py
FEATURES_LOAD_DIR_RESNET = "/gpfs/helios/home/soliman/logic_explained_networks/LR-XFL/features/ade20k_predicted_concepts" # Adjust if needed
MODEL_ARCH_FN_PART_RESNET = "ade20k_concept_predictor_resnet18" # From Stage 1 that generated these features
...
SCENES_FOR_PIPELINE_CONSISTENCY = ['street', 'bedroom', 'living_room', 'bathroom', 'kitchen',
...
scene_fn_part_resnet = '_'.join([s.replace("_", "") for s in SCENES_FOR_PIPELINE_CONSISTENCY])[:50]
dataset_name = "ade20k" # For logging and results naming
...
RESNET_OBJECT_NAMES_NPY_NAME = f"{MODEL_ARCH_FN_PART_RESNET}_binary_object_names.npy" # These are the 150 object names
...
SEED = 42
TEST_SPLIT_RATIO_CENT = 0.2 # Use the same split ratios as LR-XFL fair comparison

def generate_config_AutoCore_resnet(run_id_base=f"AutoCore_Cent_ResNetConcepts_{dataset_name}"):
    """
    Generate configuration for centralized AutoCore with ResNet18 concepts.
    This includes paths, parameters, and logging setup.
    """
    effective_run_id = f"{run_id_base}_{MODEL_ARCH_FN_PART_RESNET}_{SEED}" # More specific run_id
    ...
    config = {
        ...
        "num_classes": len(SCENES_FOR_PIPELINE_CONSISTENCY), # Default to 15, will adjust if needed
    }
    return config

def main_AutoCore_centralized_resnet():
    """
    Main function to run centralized AutoCore with ResNet18 concepts.
    This function sets up logging, loads pre-computed ResNet18 concept features,
    splits the data, and performs a hyperparameter sweep for FIGS.
    """
    ...
    object_concept_names_loaded = np.load(RESNET_OBJECT_NAMES_PATH, allow_pickle=True).tolist() # List of 150 object names
    ...
    max_rules_values = config.get("figs_max_rules_sweep", [20, 30, 40, 60, 100])
    ...
    for rules_val in max_rules_values:
        for trees_val in max_trees_values:
            for features_val in max_features_values:
                ...
                main_logger.info(f"Testing AutoCore with params: {current_figs_params}")
                figs_model = PatchedFIGSClassifier(**current_figs_params, n_outputs_global=config['num_classes'])
                ...
                try:
                    ...
                    if X_test_concepts.shape[0] > 0:
                        ...
                        # Use LR-XFL style metrics function
                    ...
                    main_logger.error(f"  Error training/evaluating AutoCore with {current_figs_params}: {e_figs_fit}", exc_info=True)
                main_logger.info("-" * 30)
