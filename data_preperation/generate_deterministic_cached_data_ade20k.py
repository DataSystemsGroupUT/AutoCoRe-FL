...
DATA_GENERATION_CONFIG = {
    "data_root_for_npy": "/gpfs/helios/home/soliman/logic_explained_networks/data/ade20k/ADEChallengeData2016/images", # For sceneCategories.txt
    "scene_cat_file": "/gpfs/helios/home/soliman/logic_explained_networks/data/ade20k/ADEChallengeData2016/sceneCategories.txt",
    # Paths to your pre-generated full dataset .npy files
    "npy_full_dataset_base_path": "/gpfs/helios/home/soliman/logic_explained_networks/experiments/", # ADJUST THIS
    "npy_all_masks_filename": "logic_ade20k_all_masksxx.npy",
    "npy_all_images_rgb_filename": "logic_ade20k_all_imagesxx.npy",
    "npy_segment_infos_filename": "logic_ade20k_segment_infosxx.npy",
    ...
    "random_seed": 42,
    "output_cache_main_dir": "./generated_autocore_fl_caches_reused_npy_10clients_ade20k",
    "run_tag_for_cache_paths": "ade20k_reused_npy",
...
def main_data_generation(config: dict):
    ...
    ade20k_prepared_images_root = os.path.join(config["data_root_for_npy"], "training")
    if not os.path.isdir(ade20k_prepared_images_root):
        main_logger.error(f"ade20k (OTF): Prepared image directory not found: {ade20k_prepared_images_root}. "
                            "Run prepare_sunrgbd_subset_scenes.py first. Exiting.")
        return
    try:
        full_masks_path = os.path.join(config["npy_full_dataset_base_path"], config["npy_all_masks_filename"])
        full_images_path = os.path.join(config["npy_full_dataset_base_path"], config["npy_all_images_rgb_filename"])
        full_seg_infos_path = os.path.join(config["npy_full_dataset_base_path"], config["npy_segment_infos_filename"])

        full_dataset_all_masks = np.load(full_masks_path, allow_pickle=True)
        full_dataset_all_images_rgb = np.load(full_images_path, allow_pickle=True)
        full_dataset_segment_infos = np.load(full_seg_infos_path, allow_pickle=True) # Flat list of dicts
        ...
        return
    main_logger.info(f"ade20k (OTF): Gathering image paths from prepared subset: {ade20k_prepared_images_root}")
    for scene_folder_name in config["chosen_classes"]:
        scene_folder_path_actual = os.path.join(ade20k_prepared_images_root, scene_folder_name.replace(" ", "_").replace("/", "_"))
    ...
    # --- 2. Prepare Image Labels and Filter by Chosen Classes ---
